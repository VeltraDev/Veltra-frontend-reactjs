...
import { submitForm, getAllForms } from './PermissionAPI';
...
const AddPermission: React.FC = () => {
  const [form, setForm] = useState<FormType>({ ...PermissionModel, modules: [] });
  const [toast, setToast] = useState<ToastMessage>({ message: '', status: '' });
  ...
  useEffect(() => {
    const fetchAllModules = async () => {
      try {
        ...
        const allModules = response.data.data?.results?.map((item: { module: string }) => item.module) || [];
  ...
  const handleOptionClick = (module: string) => {
  ...
  return (
    <section className='mt-10'>
      ...
      <Link className="mb-8 bg-[#f5f5f5] shadow hover:bg-slate-600 rounded-lg w-12 h-12 flex" to="/dashboard/permission">
